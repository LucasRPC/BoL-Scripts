...
function OnLoad()
    ...
    TS = TargetSelector(TARGET_LESS_CAST_PRIORITY, 700, DAMAGE_PHYSICAL)
    Menu = scriptConfig(ScriptName.." by "..Author, ScriptName.."24052015")

    Q = _Spell({Slot = _Q, DamageName = "Q", Range = 400, Width = nil, Delay = 0.25, Speed = math.huge, Collision = false, Aoe = false, SPELL_TYPE.TARGETTED}):AddDraw()
    W = _Spell({Slot = _W, DamageName = "W", Range = 150, Width = nil, Delay = 0.5, Speed = math.huge, Aoe = false}):AddDraw()
    ...
    R = _Spell({Slot = _R, DamageName = "R", Range = 700, Width = nil, Delay = nil, Speed = math.huge, Collision = false, Aoe = false, Type = SPELL_TYPE.TARGETTED}):AddDraw()
    ...
    Menu:addSubMenu(myHero.charName.." - Combo Settings", "Combo")
        ...
        Menu.Combo:addParam("useR", "Use R(Risky)", SCRIPT_PARAM_ONOFF, false)
...
function Combo()
    ...
    if ValidTarget(target) then
        ...
        if Menu.Combo.useR and enemy.health/enemy.maxHealth <= 0.5 then
            R:Cast(target)
        end
    end
end
